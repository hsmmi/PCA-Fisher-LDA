...
def z_score(mat):
    ...
    If you decrease scaler from all elements, the variance doesn't change so
    first decrease all elements by their column mean and then divide it by
    their column variance
    """
...
def range_min_to_max(mat, newMin, newMax):
    ...
    """
    with np.errstate(divide='ignore', invalid='ignore'):
        return ((mat - mat.min(axis=0)) / (mat.max(axis=0) - mat.min(axis=0)) *
...
def clipping(mat, newMin, newMax):
    """
    Clipping to a range:
    Get a matrix(or vector) and return matrix(or vector) that set values more
    than newMax to newMax and values less than newMin to newMin
    It's good when:
